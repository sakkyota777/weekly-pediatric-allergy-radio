...
| 症状 | 確認 |
|---|---|
| 月曜朝に何も来ない | Routines 画面で実行履歴があるか。無いならスケジュール無効／タイムゾーンずれ |
| GitHub には `reports/<日付>/` があるが Notion に無い | Connectors に Notion があるか。ハブページへの書き込み許可。実行ログの Notion エラー |
| 音声だけ無い | 環境の `GEMINI_API_KEY`（引用符なし） |
| 毎週欠ける | GitHub の Issues に `[配信ウォッチ]` が立つか確認（月曜 07:30 JST の自動チェック） |

復旧の最短手順: Routines の **今すぐ実行** → Notion ハブに「YYYY-MM-DD 号」ができることまで確認。
